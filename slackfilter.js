...
// In-memory banned phrases per channel
...
async function getChannelManagers(channelId) {
  ...
  try {
    const response = await fetch('https://slack.com/api/admin.roles.entity.listAssignments', {
      ...
      body: formData,
      headers: {
        Cookie: `d=${encodeURIComponent(process.env.XOXD || '')}`,
      },
    });
  ...
  if (bannedPhrase) {
    try {
      ...
      await slack.chat.postEphemeral({
        ...
        user: event.user,
        text: `:no_entry: The phrase *${bannedPhrase}* is banned in this channel. Your message was deleted.`,
      });
    } catch (err) {
  ...
  if (command === '/add-banned') {
    if (!text || !text.trim()) {
      return res.json({
        ...
        text: 'Please provide one or more comma-separated phrases to ban.',
      });
    }
    ...
    if (!allowed) {
      return res.json({
        response_type: 'ephemeral',
        text: '❌ Only channel managers or creators can add banned phrases.',
      });
    }
    ...
    if (!bannedWordsByChannel.has(channel_id)) {
      bannedWordsByChannel.set(channel_id, new Set());
    }
    ...
    return res.json({
      response_type: 'ephemeral',
      text: `Added banned phrases for this channel: ${phrases.join(', ')}`,
    });
  }
